...
import { UnexpectedError, type IDataObject, calculateWorkflowChecksum } from 'n8n-workflow';
...
export class WorkflowsController {
	...
	async create(req: AuthenticatedRequest, _res: unknown, @Body body: CreateWorkflowDto) {
		...
		return { ...savedWorkflowWithMetaData, scopes, nxtwavePublish, checksum };
	}
	...
	async getWorkflow(req: WorkflowRequest.Get) {
		...
		if (this.license.isSharingEnabled()) {
			...
			return { ...workflowWithMetaData, scopes, nxtwavePublish, checksum };
		}
		...
		return { ...workflow, scopes, nxtwavePublish, checksum };
	}
